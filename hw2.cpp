...
void problem1() {
	...
	{	glPushMatrix();
		glTranslatef(cos(i*36*3.14/180),sin(i*36*3.14/180),0); //10 teapots shown in the graph, so assume we have a polar coordinate system with radius 1. Starts from 3 o'clock, rotates 										   //countercolckwise, so the coordinate for each teapot is (1,360/10*0),(1,360/10*1),......(1,360/10*9), transfer to x-y coordinate
									//it becomes (cos angle, sin angle) where angle stands for the angle of each teapot, and since sin, cos in cmath use arc system,
...
void problem2() {
	for(int i=0;i<=7;i++) // left part
	{
		glPushMatrix();
		glScalef(1,pow(2,0.4*(0.3*8-0.3*i)),1);	//Scaling cubes only on y-axis. Since the top is not a stright line, I use the curve of an exponential function (2^0.4x) to represent the top.
		glTranslatef(-0.3*i-0.3/2,0.15,0);	//Move the scaled cubes on x-axis 0.3*i each time to let them stand adjacent to each other, and move each 0.15 on y-axis to align their bottom.
							//Since the origin locates between the 8th and 9th cube, we need to move a/2 units each side initially, where a is the size of cube.
		glutSolidCube(0.3);			
		glPopMatrix();
	}
	for(int i=0;i<7;i++) // right part
	{
		glPushMatrix();
		glScalef(1,pow(2,0.4*(0.3*9+0.3*i)),1);
		glTranslatef(0.3*i+0.3/2,0.15,0);
		glutSolidCube(0.3);
		glPopMatrix();
	}
